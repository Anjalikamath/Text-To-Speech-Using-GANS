...
class Hps(object):
    def __init__(self):
        ...
        )#50000, 5000, 5000, 30000, 60000
        default = \
            [1e-4, 1, 1e-4, 0, 0, 0, 10, 0.01, 0.5, 0.1, 5, 5, 128, 128, 8, 5, 0, 32, 5000, 500, 500, 3000, 6000]
...
class Indexer(object):
    ...
    def index(self, speaker_id, utt_id, dset='train', feature='mc'):
        return self.h5['{}/{}/{}/{}'.format(dset,speaker_id,utt_id,feature)][:]
        
    def get_mean_std(self, speaker_id, feature='mc'):
        mean = self.norm_h5['{}/{}_mean'.format(speaker_id,feature)]
        std = self.norm_h5['{}/{}_std'.format(speaker_id,feature)]
        return mean, std
...
class Sampler(object):
    def __init__(
       ...
       #print(dset)
        ...
        self.speaker2utts = {speaker:list(self.f_h5['{}/{}'.format(dset,speaker)].keys()) \
    ...
    def get_utt_len(self):
        ...
        for dset in ['train', 'test']:
            for speaker in self.f_h5['{}'.format(dset)]:
                for utt_id in self.f_h5['{}/{}'.format(dset,speaker)]:
                    length = self.f_h5['{}/{}/{}/lin'.format(dset,speaker,utt_id)][()].shape[0]
    ...
    def sample_utt(self, speaker_id, n_samples=1):
        ...
        lengths = [self.f_h5['{}/{}/{}/lin'.format(dset,speaker_id,utt_id)].shape[0] for utt_id in utt_ids]
    ...
    def sample_single(self):
        ...
        index_tuple = self.single_indexer(speaker=speaker_idx, i='{}/{}'.format(speaker,utt_id), t=t)
        return index_tuple
    ...
    def sample(self):
        ...
        index_tuple = self.indexer(speaker_i=speakerA_idx, speaker_j=speakerB_idx,\
                i0='{}/{}'.format(speakerA,A_utt_id_0), i1='{}/{}'.format(speakerA,A_utt_id_1),\
                j='{}/{}'.format(speakerB,B_utt_id), t=t, t_k=t_k, t_prime=t_prime, t_j=t_j)
        return index_tuple
...
class SingleDataset(data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        i, t = index.i, index.t
        seg_len = self.seg_len
        data = [speaker, self.dataset['{}/{}/lin'.format(self.dset,i)][t:t+seg_len]]
        return tuple(data)
...
class myDataset(data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        data.append(self.h5['{}/{}/lin'.format(self.dset,i0)][t:t+seg_len])
        data.append(self.h5['{}/{}/lin'.format(self.dset,i0)][t_k:t_k+seg_len])
        data.append(self.h5['{}/{}/lin'.format(self.dset,i1)][t_prime:t_prime+seg_len])
        data.append(self.h5['{}/{}/lin'.format(self.dset,j)][t_j:t_j+seg_len])
        return tuple(data)
