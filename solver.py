...
class Solver(object):
    ...
    def load_model(self, model_path, enc_only=True):
        ...
        with open(model_path, 'rb') as f_in:
            ...
            return self.Encoder,self.Decoder,self.Generator
            if not enc_only:
                ...
                return self.Encoder,self.Decoder,self.Generator,self.SpeakerClassifier,self.PatchDiscriminator
            #iteration=all_model.split('-')
            #iteration=iteration[1]
            iteration=2000
            print("=> loaded checkpoint '{}' (epoch {})".format(all_model, iteration))
    ...
    def train(self, model_path, flag='train', mode='train'):
        ...
        if mode == 'pretrain_G':
            for iteration in range(2200):
                ...
                slot_value = (iteration + 1,2200) + tuple([value for value in info.values()])
            ...
            for iteration in range(1100):
                ...
                for step in range(hps.n_patch_steps):
                    ...
                    slot_value = (step, iteration+1, 1100) + tuple([value for value in info.values()])
                ...
                slot_value = (iteration+1, 1100) + tuple([value for value in info.values()])
